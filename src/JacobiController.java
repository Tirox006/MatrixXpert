...
public class JacobiController {
    ...
    private void handleRetrieveAndCalculateButton() {
        ...
        Vector v= new Vector(vector);
         ...
         int nbIteration = nbIt.getText().isEmpty() ? 0 : Integer.parseInt(nbIt.getText());

        if(m.t.type==0 && nbIteration==0 ){

               Vector productVector = Resolution.Jacobi(m,v);
               rq.setTextFill(Color.GREEN);
               rq.setText("Valid Matrix");



               productVector.afficher();
        ...
        }else if(m.t.type==0 && nbIteration!=0){
            Vector productVector = Resolution.Jacobi(m,v,nbIteration);
               rq.setTextFill(Color.GREEN);
               rq.setText("Valid Matrix");
