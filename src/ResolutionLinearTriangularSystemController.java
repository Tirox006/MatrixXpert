...
public class ResolutionLinearTriangularSystemController {
    private Stage stage;
    private Scene scene;
    private Parent root; 
    @FXML
    ...
    private void handleRetrieveAndCalculateButton() {
        ...
        for (int i = 0; i < matrixGridPane.getRowCount(); i++) {
            for (int j = 0; j < matrixGridPane.getColumnCount(); j++) {
                ...
                matrix[i][j] = (float) (textField.getText().isEmpty() ? 0.0 : Float.parseFloat(textField.getText()));
            }
        }
        ...
        for (int i = 0; i < vectorGridPane.getRowCount(); i++) {
            ...
            vector[i] = (float) (vectorTextField.getText().isEmpty() ? 0.0 : Float.parseFloat(vectorTextField.getText()));
        }
        ...
        if(m.t.type==1 || m.t.type==2 || m.t.type==3 || m.t.type==4 ){

               Vector productVector = ResolutionTriangulair.resolutionTriangulaire(m,v);
    ...
    private void displayVectorInGridPane(Vector productVector, GridPane gridPane) {
        gridPane.getChildren().clear();

        for (int i = 0; i < productVector.size; i++) {
            TextField textField = new TextField(Float.toString(productVector.vect[i]));
